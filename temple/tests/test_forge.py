...
def test_gitlab_get_client(gitlab_api_token, mocker):
    """Tests temple.forget.Gitlab.get_client()"""
    mocker.patch.dict(os.environ, {"GITLAB_API_TOKEN": gitlab_api_token}, clear=True)

    assert temple.forge.Gitlab().get_client("https://gitlab.com")


def test_gitlab_get_gitlab_url_and_repo_path():
    """Tests temple.forge.Gitlab._get_gitlab_url_and_repo_path"""
    assert temple.forge.Gitlab()._get_gitlab_url_and_repo_path(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "gitlab.com",
            None,
            marks=pytest.mark.xfail(raises=temple.exceptions.InvalidGitlabGroupError),
        ),
    ],
)
def test_gitlab_get_gitlab_url_and_group(forge, expected_results):
    """Tests temple.forge.Gitlab._get_gitlab_url_and_group"""
    assert temple.forge.Gitlab()._get_gitlab_url_and_group(forge) == expected_results
...
def test_get_latest_template_version_w_ssh(mocker, stdout, stderr, expected):
    ...
    mock_shell = mocker.patch(
        "temple.utils.shell", autospec=True, return_value=ls_remote_return
    )
...
def test_github_get_latest_template_version(
    ...
    mocker.patch(
        "temple.forge._get_latest_template_version_w_ssh",
        autospec=True,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gitlab.com/user", temple.forge.Gitlab),
        pytest.param(
            "invalid",
            None,
            marks=pytest.mark.xfail(raises=temple.exceptions.InvalidRootError),
        ),
    ],
)
...
def test_github_parse_link_header(headers, expected_links):
    """Tests temple.forge.Github._parse_link_header"""
...
def test_github_code_search_multiple_pages(mocker, responses):
    """Tests temple.forge.Github._code_search for a single page of responses"""
    response_content1 = {
        "items": [
            {
                "repository": {
...
def test_github_code_search_invalid_root(mocker, responses):
    """Tests temple.forge.Github._code_search when the root is invalid"""
    responses.add(
        responses.GET,
        "https://api.github.com/search/code",
